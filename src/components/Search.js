    ...
    <>
      ...
      >
        <input
          ...
          className="rounded-2xl w-full rounded bg-gray-50 placeholder:text-gray-100 pl-2 placeholder:text-base required outline-0 
border border-transparent focus:border-black
"
        />
        <button type="submit" className="absolute right-1 cursor-pointer">
          <img src={searchIcon} alt="CryptoBucks" className="w-full h-auto fill-black " />
        </button>
      </form>

      {searchText.length > 0 ? (
        <ul
          className="absolute top-11 right-0 lg:w-96 w-full bg-gray-50 rounded overflow-x-hidden  py-2 
        ...
        >
          {searchData ? (
            ...
            <div className=" w-full h-full flex justify-center items-center">
              <div
                className="w-8 h-8 border-4 border-solid border-black rounded-full border-b-gray-200 animate-spin"
                role="status"
              />
