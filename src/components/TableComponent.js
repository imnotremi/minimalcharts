    ...
    >
      <svg
        className={`w-[1.5rem] ml-1.5 ${
          allCoins.includes(data.id) ? "fill-black" : "fill-gray-100"
        } hover:fill-black`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
...
const TableComponent = () => {
  ...
  return (
    <>
              ...
              {cryptoData &&
                cryptoData.map((data) => {
                  return (
                    <tr
                      key={data.id}
                      className="hover:bg-white rounded-2xl text-center text-base text-black"
                    >
                      <td className="hover:rounded-2xl hover:scale-[103%] ease-in duration-200 rounded-2xl py-4 uppercase font-semibold flex items-center">
                        <SaveBtn data={data} />
                        <img
                          src={data.image}
                          alt={data.id}
                      ...
                      <td className="py-4 ">
                        ...
                        {new Intl.NumberFormat("en-IN", {
                          style: "currency",
                          currency: currency,
                          minimumFractionDigits: data.current_price >= 1 ? 2 : 5,
                      ...
                      <td
                        className={
                          data.price_change_percentage_7d_in_currency < 0
                            ? "rounded-r-2xl py-4 lg:table-cell hidden text-red"
                            : "rounded-r-2xl py-4 lg:table-cell hidden text-green"
                        }
                      >
                        {Number(
      ...
      <div className="text-black flex md:flex-row flex-col items-center justify-between  mt-4 capitalize h-[2rem] ">
        <span>
          Data provided by{" "}
          <a
